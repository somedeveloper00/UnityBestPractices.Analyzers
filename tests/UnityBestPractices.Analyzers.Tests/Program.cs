...
using UnityBestPractices.Analyzers.Testing;
...
{
    private const string UnityStubs = UnityTestSources.UnityAndDots;
    ...
    {
        var fixtureValidation = await AnalyzerTestHost.ValidateUnityFixturesAsync();
        if (!fixtureValidation.Succeeded)
            throw new InvalidOperationException($"Shared Unity fixtures did not compile: {FormatDiagnostics(fixtureValidation.Diagnostics)}");
    ...
    private static IEnumerable<MetadataReference> GetPlatformReferences() => AnalyzerTestHost.PlatformReferences;
