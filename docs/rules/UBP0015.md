...
The recognized form is `\`new Vector2(0f, -1f)\``. The quick fix uses `Uses \`UnityEngine.Vector2.down\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Vector2.down":
// Uses `UnityEngine.Vector2.down`
```
