...
The recognized form is `\`new Vector3(1f, 0f, 0f)\``. The quick fix uses `Uses \`UnityEngine.Vector3.right\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Vector3.right":
// Uses `UnityEngine.Vector3.right`
```
