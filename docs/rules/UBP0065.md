# UBP0065: Switch IJobEntity from Run to Schedule
...
The recognized form is `A parameterless \`IJobEntity.Run()\` invocation`. The quick fix uses `Switches execution to \`Schedule()\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
