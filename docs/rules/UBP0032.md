...
The recognized form is `\`new Color(0f, 1f, 0f, 1f)\``. The quick fix uses `Uses \`UnityEngine.Color.green\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Color.green":
// Uses `UnityEngine.Color.green`
```
