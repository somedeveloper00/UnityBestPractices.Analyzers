# UBP0051: Use Array.Length for emptiness
...
The recognized form is `\`array.Any()\` on a one-dimensional array`. The quick fix uses `Uses \`array.Length != 0\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.

## Before

```csharp
// Simplified recognized form:
// `array.Any()` on a one-dimensional array
```

## After

```csharp
// Simplified result of "Use Array.Length for emptiness":
// Uses `array.Length != 0`
```
