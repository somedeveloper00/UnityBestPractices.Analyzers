# UBP0052: Use List.Count for emptiness
...
The recognized form is `\`list.Any()\` on a concrete \`List<T>\``. The quick fix uses `Uses \`list.Count != 0\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.

## Before

```csharp
// Simplified recognized form:
// `list.Any()` on a concrete `List<T>`
```

## After

```csharp
// Simplified result of "Use List.Count for emptiness":
// Uses `list.Count != 0`
```
