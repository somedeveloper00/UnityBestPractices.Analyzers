...
The recognized form is `\`new Color(0f, 0f, 1f, 1f)\``. The quick fix uses `Uses \`UnityEngine.Color.blue\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Color.blue":
// Uses `UnityEngine.Color.blue`
```
