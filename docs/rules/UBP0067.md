# UBP0067: Switch IJobEntity from Schedule to Run
...
The recognized form is `A parameterless \`IJobEntity.Schedule()\` invocation`. The quick fix uses `Switches execution to \`Run()\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
