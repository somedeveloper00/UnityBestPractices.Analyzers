# UBP0059: Convert Entities.ForEach to IJobEntity.Run
...
The recognized form is `A compatible \`Entities.ForEach\` pipeline`. The quick fix uses `Extracts a Burst-enabled \`IJobEntity\` and invokes \`Run()\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Convert to IJobEntity.Run":
// Extracts a Burst-enabled `IJobEntity` and invokes `Run()`
```
