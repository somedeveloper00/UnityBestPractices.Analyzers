...
The recognized form is `Construction of Unity's standard yellow RGBA value`. The quick fix uses `Uses \`UnityEngine.Color.yellow\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.

## Before

```csharp
// Simplified recognized form:
// Construction of Unity's standard yellow RGBA value
```

## After

```csharp
// Simplified result of "Use Color.yellow":
// Uses `UnityEngine.Color.yellow`
```
