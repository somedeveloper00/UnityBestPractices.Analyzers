# UBP0069: Switch IJobEntity from ScheduleParallel to Run
...
The recognized form is `A parameterless \`IJobEntity.ScheduleParallel()\` invocation`. The quick fix uses `Switches execution to \`Run()\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Run execution":
// Switches execution to `Run()`
```
