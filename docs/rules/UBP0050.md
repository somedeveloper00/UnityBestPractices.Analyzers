...
The recognized form is `\`array.Count()\` on a one-dimensional array`. The quick fix uses `Uses the \`array.Length\` property` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.

## Before

```csharp
// Simplified recognized form:
// `array.Count()` on a one-dimensional array
```

## After

```csharp
// Simplified result of "Use Array.Length":
