# UBP0026: Use Quaternion.identity
...
The recognized form is `\`new Quaternion(0f, 0f, 0f, 1f)\``. The quick fix uses `Uses \`UnityEngine.Quaternion.identity\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
