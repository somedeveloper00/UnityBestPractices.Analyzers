...
The recognized form is `\`(int)Mathf.Floor(value)\``. The quick fix uses `Uses \`UnityEngine.Mathf.FloorToInt(value)\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Mathf.FloorToInt":
// Uses `UnityEngine.Mathf.FloorToInt(value)`
```
