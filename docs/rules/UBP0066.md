# UBP0066: Switch IJobEntity from Run to ScheduleParallel
...
The recognized form is `A parameterless \`IJobEntity.Run()\` invocation`. The quick fix uses `Switches execution to \`ScheduleParallel()\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
// A parameterless `IJobEntity.Run()` invocation
```
