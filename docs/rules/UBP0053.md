...
The recognized form is `\`StringBuilder.Append("x")\` with a one-character constant`. The quick fix uses `Uses the character overload \`Append('x')\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.

## Before

```csharp
// Simplified recognized form:
// `StringBuilder.Append("x")` with a one-character constant
```

## After

```csharp
// Simplified result of "Use Append(char)":
// Uses the character overload `Append('x')`
```
