...
The recognized form is `\`new Vector3(1f, 1f, 1f)\``. The quick fix uses `Uses \`UnityEngine.Vector3.one\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Vector3.one":
// Uses `UnityEngine.Vector3.one`
```
