# UBP0063: Convert SystemAPI.Query to IJobEntity.Schedule
...
The recognized form is `A compatible \`SystemAPI.Query\` foreach loop`. The quick fix uses `Extracts a Burst-enabled \`IJobEntity\` and invokes \`Schedule()\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
