...
The recognized form is `\`new Color(1f, 0f, 0f, 1f)\``. The quick fix uses `Uses \`UnityEngine.Color.red\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Color.red":
// Uses `UnityEngine.Color.red`
```
