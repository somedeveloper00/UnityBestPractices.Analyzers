...
The recognized form is `\`(int)Mathf.Round(value)\``. The quick fix uses `Uses \`UnityEngine.Mathf.RoundToInt(value)\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Mathf.RoundToInt":
// Uses `UnityEngine.Mathf.RoundToInt(value)`
```
