# UBP0054: Append an empty line directly
...
The recognized form is `\`StringBuilder.AppendLine("")\``. The quick fix uses `Uses parameterless \`AppendLine()\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use AppendLine()":
// Uses parameterless `AppendLine()`
```
