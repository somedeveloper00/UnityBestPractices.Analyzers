# UBP0046: Fuse Where with FirstOrDefault
...
The recognized form is `\`source.Where(predicate).FirstOrDefault()\` with a non-indexed predicate`. The quick fix uses `Uses \`source.FirstOrDefault(predicate)\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.

## Before

```csharp
// Simplified recognized form:
// `source.Where(predicate).FirstOrDefault()` with a non-indexed predicate
```

## After

```csharp
// Simplified result of "Use FirstOrDefault with the predicate":
// Uses `source.FirstOrDefault(predicate)`
```
