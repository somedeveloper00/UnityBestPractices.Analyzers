...
The recognized form is `\`list.ElementAt(index)\` on a concrete \`List<T>\``. The quick fix uses `Uses \`list[index]\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.

## Before

```csharp
// Simplified recognized form:
// `list.ElementAt(index)` on a concrete `List<T>`
```

## After

```csharp
// Simplified result of "Use the List indexer":
// Uses `list[index]`
```
