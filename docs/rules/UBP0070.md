# UBP0070: Switch IJobEntity from ScheduleParallel to Schedule
...
The recognized form is `A parameterless \`IJobEntity.ScheduleParallel()\` invocation`. The quick fix uses `Switches execution to \`Schedule()\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
// A parameterless `IJobEntity.ScheduleParallel()` invocation
```

## After

```csharp
// Simplified result of "Use Schedule execution":
// Switches execution to `Schedule()`
```
