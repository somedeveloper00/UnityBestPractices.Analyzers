...
The recognized form is `\`new Guid()\``. The quick fix uses `Uses \`System.Guid.Empty\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Guid.Empty":
// Uses `System.Guid.Empty`
```
