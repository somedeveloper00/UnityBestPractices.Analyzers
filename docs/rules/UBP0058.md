...
- Synchronous conversions support local and instance captures, `WithoutBurst()`, component parameters, and entity-only lambdas constrained by at least one `WithAll<T>()` filter. Entity-only `WithStructuralChanges()` loops use a disposable entity snapshot so structural edits cannot invalidate iteration.
