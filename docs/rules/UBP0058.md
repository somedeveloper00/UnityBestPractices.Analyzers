...
- Nested lambdas, unresolved or unsupported filters, entity-only loops without a usable query component, and structural-change loops that expose component references are excluded. Scheduled job conversions remain unavailable for managed or mutated captures, unsupported `SystemAPI` access, `WithoutBurst()`, or structural-change pipelines.
