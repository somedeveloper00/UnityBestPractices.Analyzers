...
The recognized form is `\`new Vector3(0f, 0f, 1f)\``. The quick fix uses `Uses \`UnityEngine.Vector3.forward\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Vector3.forward":
// Uses `UnityEngine.Vector3.forward`
```
