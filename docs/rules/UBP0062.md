# UBP0062: Convert SystemAPI.Query to IJobEntity.Run
...
The recognized form is `A compatible \`SystemAPI.Query\` foreach loop`. The quick fix uses `Extracts a Burst-enabled \`IJobEntity\` and invokes \`Run()\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
