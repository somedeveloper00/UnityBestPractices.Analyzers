# UBP0027: Use Quaternion.identity
...
The recognized form is `\`Quaternion.Euler(0f, 0f, 0f)\``. The quick fix uses `Uses \`UnityEngine.Quaternion.identity\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Quaternion.identity":
// Uses `UnityEngine.Quaternion.identity`
```
