...
The recognized form is `\`new Vector2(1f, 0f)\``. The quick fix uses `Uses \`UnityEngine.Vector2.right\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Vector2.right":
// Uses `UnityEngine.Vector2.right`
```
