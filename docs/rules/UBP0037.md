...
The recognized form is `\`Mathf.Clamp(value, 0f, 1f)\``. The quick fix uses `Uses \`UnityEngine.Mathf.Clamp01(value)\`` to express the documented Unity or C# best practice while retaining the rule's semantic preconditions.
...
```csharp
// Simplified result of "Use Mathf.Clamp01":
// Uses `UnityEngine.Mathf.Clamp01(value)`
```
